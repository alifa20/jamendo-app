...
pnpm test  # Tests MUST FAIL (red phase)
...
pnpm test  # Tests MUST PASS (green phase)
...
6. **Build & Tests**: `pnpm lint && pnpm build` and `pnpm test` must pass
...
8. **Build**: Run `pnpm lint && pnpm build` to ensure quality standards met
...
fspec add-virtual-hook AUTH-001 post-implementing "pnpm lint && pnpm build" --blocking
...
**Example**:
```bash
# Work unit AUTH-001 has virtual hook: "pnpm lint && pnpm build"
...
# Execution order:
# 1. AUTH-001 virtual hook: pnpm lint && pnpm build
...
fspec add-virtual-hook AUTH-001 pre-validating "pnpm lint && pnpm build" --blocking
...
**Quality gates**:
```bash
fspec add-virtual-hook AUTH-001 post-implementing "pnpm test" --blocking
fspec add-virtual-hook AUTH-001 pre-validating "eslint" --git-context --blocking
```
...
Next steps:
  ...
  3. Run tests to validate: pnpm test
